...
import xyz.bluspring.unitytranslate.gui.elementa.ElementaUIHelpers.expandableSection
import xyz.bluspring.unitytranslate.gui.elementa.ElementaUIHelpers.fileChooser
    ...
    val sections = ScrollComponent().constrain {
        ...
        expandableSection(ChatColor.BOLD + i18n("gui.unitytranslate.config.client")) {
            ...
            toggleButton("Config Background", config.backgroundEnabled) {
                ...
                displayScreen(UTConfigScreen(this@UTConfigScreen))
            }
                .constrain { defaultButtonConstraints() } childOf this
            ...
            button("Edit Transcript Boxes")
                ...
                .onMouseClick {
                    LayeredScreenManager.open(EditTranscriptBoxesScreen())
                } childOf this

            button("Set Spoken Language (${i18n(config.spokenLanguage.translationKey)})")
                .constrain { defaultButtonConstraints() }
                .onMouseClick {
                    displayScreen(LanguageSelectScreen({ language ->
                        config.spokenLanguage = language!!
                    }))
                } childOf this

            button("Set Balloon Language (${i18n(if (config.balloonLanguage == null) config.spokenLanguage.translationKey else config.balloonLanguage!!.translationKey)})")
                .constrain { defaultButtonConstraints() }
                .onMouseClick {
                    displayScreen(LanguageSelectScreen({ language ->
            ...
            for (index in 0 until 5) {
                button("Language Profile ${index + 1}: ${i18n(if (config.languageProfiles.getOrNull(index) == null) "unitytranslate.value.none" else config.languageProfiles[index]!!.translationKey)}")
                    .constrain { defaultButtonConstraints() }
                    .onMouseClick {
                        displayScreen(LanguageSelectScreen({ language ->
