...
data class TranscriptBoxConfig(
    ...
    sealed class Background(val type: String) {
        companion object {
            @JvmStatic
            fun default(): Background = Color(ColorConfig.Gradient(GradientComponent.GradientDirection.TOP_TO_BOTTOM,
    ...
    sealed class TranscriptDisplay(val type: String) {
        ...
        data class LangCode(val style: Style = DEFAULT_STYLE) : TranscriptDisplay("lang_code") {
            // <BluSpring (en)> Hi
            override fun text(data: TranscriptData): Component = Component.translatable(MESSAGE_LANG, data.displayName,
