...
object Keyboard {
    fun keyPress(handle: Long, action: Int, event: KeyEvent) {
        if (handle == UnityTranslateStandalone.window.handle()) {
            UnityTranslateElementaGui.keyPress(event.key, 0.toChar(), null)
        }
    }
    ...
    fun charTyped(handle: Long, event: CharacterEvent) {
        if (handle == UnityTranslateStandalone.window.handle()) {
            if (Character.isBmpCodePoint(event.codepoint)) {
                UnityTranslateElementaGui.keyPress(0, event.codepoint.toChar(), null)
            } else if (Character.isValidCodePoint(event.codepoint)) {
                UnityTranslateElementaGui.keyPress(0, Character.highSurrogate(event.codepoint), null)
                UnityTranslateElementaGui.keyPress(0, Character.lowSurrogate(event.codepoint), null)
            }
        }
    }
}
